...
def _normalize_prediction(result):
    """Normalize prediction values for consistent UI display."""
    raw_label = str(result.get("predicted_label", "nature")).strip().lower()
    label_display = "ANTHRO" if raw_label == "anthro" else "NATURE"

    raw_confidence = result.get("confidence", 0.0)
    if isinstance(raw_confidence, str):
        confidence_level = raw_confidence.upper()
        confidence_score = float(
            max(result.get("prob_nature", 0.0), result.get("prob_anthro", 0.0))
        )
    else:
    ...
    return label_display, confidence_level, confidence_score
...
def plot_prediction_gauge(prob_nature, prob_anthro):
    ...
    ax.barh(
        ...
        left=prob_nature,
        height=bar_height,
        color=config.COLOR_ANTHRO,
        label=f"Anthropogenic: {prob_anthro:.1%}",
    )

    # Add threshold line
    ax.axvline(
        x=config.ANTHRO_THRESHOLD,
        color="black",
        linestyle="--",
        linewidth=2,
        alpha=0.7,
...
def create_results_summary(result, filename):
    ...
    """
    label, confidence_level, confidence_score = _normalize_prediction(result)
    ...
    confidence_icon = "TARGET" if confidence_level == "HIGH" else "CAUTION"

    html = f"""
    <div style='background-color: {color}22; padding: 20px; border-radius: 10px;
                border-left: 5px solid {color};'>
        <h2 style='color: {color}; margin-top: 0;'>
            {confidence_icon} Prediction: <strong>{label}</strong>
        </h2>
        <p style='font-size: 16px; margin: 10px 0;'>
            <strong>File:</strong> {filename}<br>
            <strong>Confidence:</strong> {confidence_level} ({confidence_score:.2%})<br>
...
def plot_batch_results(results_df):
    ...
    display_labels = (
        ...
        .astype(str)
        .str.lower()
        .map({"nature": "NATURE", "anthro": "ANTHRO"})
    )
    class_counts = display_labels.value_counts()
    colors = [
        config.COLOR_NATURE if str(label).upper() == "NATURE" else config.COLOR_ANTHRO
    ...
    ax2.hist(
        results_df["prob_anthro"],
        bins=20,
        color=config.COLOR_ANTHRO,
        alpha=0.6,
        edgecolor="black",
        label="Anthropogenic Probability",
    )
    ax2.axvline(
        x=config.ANTHRO_THRESHOLD,
        color="black",
