...
def render_results_section(result, processed_audio, filename):
    ...
    tab1, tab2, tab3 = st.tabs(["📈 Waveform", "🌈 Spectrogram", "🖼️ Model Input"])
    ...
    with tab2:
        spec_fig = plot_spectrogram(
            processed_audio['mel_spectrogram'], 
            sample_rate
        )
        st.pyplot(spec_fig)
    ...
    with tab3:
        ...
        st.image(
            ...
            caption="128×128 RGB Mel-Spectrogram",
            width='stretch'
        )
    
    # Additional info
    with st.expander("ℹ️ About the Classification"):
        st.markdown(f"""
        **How it works:**
        - The audio is converted to a mel-spectrogram (visual representation of sound)
        - The spectrogram is resized to 128×128 pixels and converted to RGB
        ...
        **Current prediction:**
        - The model is **{result['confidence'].lower()} confidence** that this is **{result['predicted_label']}** sound.
        """)
